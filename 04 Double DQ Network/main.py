...
if __name__ == "__main__":
    ...
    agt = Agent(gamma=0.99, epsilon_initial=eps_i, epsilon_decay=eps_f, epsilon_final=eps_f, memory_size=100000,
                input_dimensions=env.observation_space.shape[0], action_dimensions=env.action_space.n,
                learning_rate=0.001, weight_decay=0.0, batch_size=64, replace=100, algorithm='Double_DQN',
                environment=env.spec.id, directory="C:/Users/salwahai/Documents/Best Model")
    ...
    for i in range(esp):
        ...
        while not ter:
            ...
            if not tst:
                agt.saving(state=sta, action=act, reward=rwd, future_state=fut_sta, terminal=ter)
                agt.learn()
