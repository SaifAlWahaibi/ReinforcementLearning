...
class Agent:
    def __init__(self, gamma, epsilon_initial, epsilon_decay, epsilon_final, memory_size, input_dimensions,
        ...
        self.q_eva = DeepQNetwork(network_name=self.env + '_' + self.alg + '_Q_Eval_Network.pth', directory=self.dic,
                                  input_dimensions=self.inp_dim, action_dimensions=self.act, learning_rate=self.lr,
                                  weight_decay=self.l2_reg)

        self.q_tar = DeepQNetwork(network_name=self.env + '_' + self.alg + '_Q_Target_Network.pth', directory=self.dic,
                                  input_dimensions=self.inp_dim, action_dimensions=self.act, learning_rate=self.lr,
                                  weight_decay=self.l2_reg)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            q_fut = self.q_tar(fut_sta).max(dim=-1)[0]

        self.q_tar.train()

        q_fut[ter] = 0.0

        td_tar = rwd + self.gam * q_fut
